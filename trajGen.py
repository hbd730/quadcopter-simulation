...
def genLineY(t):
    ...
    # constant velocity
        ...
        dt = t - v_max / a_max
        vel[1] = v_max
        pos = np.array([0, v_max**2 / (2 * a_max), 0]) + np.array([0, v_max * dt, 0])
    # decelarate    
    elif t <= 3 * v_max / a_max:
        dt = t - 2 * v_max / a_max
        acc[1] = -a_max
        vel = np.array([0, v_max, 0]) + acc * dt
        pos = np.array([0, 3 * v_max**2 / (2 * a_max), 0]) + np.array([0,v_max,0]) * dt + 0.5 * acc * dt**2
    # hover 
    else:
        pos = np.array([0, 2 * v_max**2 / a_max, 0])
